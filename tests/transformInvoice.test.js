...
describe("Invoice Transformation Tests", () => {
  test("successfully transforms a valid invoice", () => {
    const result = transformInvoice("INV-001");
    expect(result.invoiceId).toBe("INV-001");
    expect(result.status).toBe("transformed");
    expect(result.invoiceXml).toContain("<Invoice");
  });

  test("invalid invoiceId", () => {
    expect(() => transformInvoice("INVALID-ID")).toThrow("Invoice not found");
  });
});
